...
// https://sunrisesunset.io/api/
    ...
    const weatherIconOutput = () => {
      ...
      const temperatureToday = (twentyfourHours) => {
      ...
      const maxTempTwentyfourHours = (twentyfourHours) => {
        // Converts the 24hr data into data that only matches todays date
        // pushes temperature data into a max temp array and looks for the highest value
        const todaysTemp = twentyfourHours.slice(0, temperatureToday())
        const maxTemp = []
        todaysTemp.forEach((items) => {maxTemp.push(parseInt(items.data.instant.details.air_temperature))})
        return Math.max(...maxTemp)
      }
    
      const minTempTwentyfourHours = (twentyfourHours) => {
        // Converts the 24hr data into data that only matches todays date
        // pushes temperature data into a max temp array and looks for the lowest value
        const todaysTemp = twentyfourHours.slice(0, temperatureToday())
        const minTemp = []
        todaysTemp.forEach((items) => {minTemp.push(parseInt(items.data.instant.details.air_temperature))})
